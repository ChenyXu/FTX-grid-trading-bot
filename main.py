...
class Grid_trader:
    ...
    def __init__(self, exchange, symbol, grid_level=0, lower_price=0.0, upper_price=0.0, amount=0, stop=0, side='long'):
        self.symbol = symbol
    ...
    def form_initial_balance(self):
        ...
        if self.side == 'long':
            self.amount = self.totalamount / ((self.upper_price - current_price)/(self.upper_price - self.lower_price)+1) / self.grid_level
            self.exchange.createMarketBuyOrder(self.symbol, (self.upper_price - current_price)/(self.upper_price - self.lower_price)*self.grid_level*self.amount)
        elif self.side == 'short':
            self.amount = self.totalamount / ((current_price - self.lower_price)/(self.upper_price - self.lower_price)+1) /self.grid_level
            self.exchange.createMarketSellOrder(self.symbol, (current_price - self.lower_price) / (self.upper_price - self.lower_price) * self.grid_level * self.amount)
        else:
            pass
